...
function TWT.getTargetLowGUID()
    -- Get both return values at once
    local exists, guid = UnitExists("target")
    
    if exists then
        if guid and type(guid) == "string" then
            -- We have a GUID string from SuperWoW
            local low = string.sub(guid, -4)
            if low then
                return tostring(tonumber(low, 16))
            end
        else
            -- No GUID string, try TWT.units table
            for low, unit in pairs(TWT.units) do
                if unit == "target" then
                    return tostring(low)
                end
            end
...
function TWT.getTargetLowGUID()
    if UnitExists("target") then
        ...
        if guid and type(guid) == "string" then
            -- Extract low GUID from full GUID (creature GUID format: 0xF1300000XXXXXX)
            -- Last 4 bytes are the low GUID
            local low = string.sub(guid, -4)
            if low then
                return tostring(tonumber(low, 16))
            end
...
function TWT.synthesizeTankModeData()
    -- Clear existing synthetic entries (keep those from tank mode API)
    for guid, data in pairs(TWT.tankModeThreats) do
        if data.synthetic then
            TWT.tankModeThreats[guid] = nil
        end
    end
    
    -- Only synthesize if we have threat data
    if TWT.tableSize(TWT.threats) == 0 then
        return false
    end
    
    -- Get target info if available
    local creatureName = TWT.targetName or ""
    local lowGuid = nil
    local syntheticKey = nil
    
    -- Try to get the target's low GUID
    if UnitExists("target") and not UnitIsPlayer("target") then
        creatureName = UnitName("target")
        local exists, fullGuid = UnitExists("target")  -- Get both returns
        if fullGuid and type(fullGuid) == "string" then
            -- Extract low GUID from full GUID
            local low = string.sub(fullGuid, -4)
            if low then
                lowGuid = tostring(tonumber(low, 16))  -- Store the low GUID
            end
        end
    end
    
    -- If we couldn't get a real low GUID, create a synthetic one
    if not lowGuid then
        syntheticKey = "synthetic_" .. creatureName
        lowGuid = syntheticKey  -- Use synthetic as both key and guid
    end
    
    -- If still no creature name, use a default
    if creatureName == "" then
        creatureName = "Current Target"
    end
    
    -- Find the top TWO threat holders (excluding AGGRO marker)
    local topName, topData, secondName, secondData
    for name, data in pairs(TWT.threats) do
        if name ~= TWT.AGRO then
            if not topData or data.perc > topData.perc then
                -- Move current top to second
                secondName = topName
                secondData = topData
                -- Set new top
                topName = name
                topData = data
            elseif not secondData or data.perc > secondData.perc then
                -- This is second highest
                secondName = name
                secondData = data
            end
        end
    ...
    -- Create synthetic entry - use SECOND highest threat holder for tank mode
    -- (since top is usually the tank themselves)
    local displayName = secondName or topName
    local displayData = secondData or topData
    
    if displayName and displayData then
        TWT.tankModeThreats[lowGuid] = {
            creature = creatureName,
            name = displayName,
            perc = TWT.round(displayData.perc),
            synthetic = true,
            threat = displayData.threat or 0,
            tps = displayData.tps or 0,
            tank = displayData.tank or false,
            isTopThreat = (displayName == topName)
        }
        return true
    end
...
function TWT.init()
    ...
    _G["TWThreat_TankModeData"] = TWT.tankModeThreats
...
function TWT.handleThreatPacket(packet)
    ...
    -- ALWAYS synthesize tank mode data when we have threat data
    -- This populates the tankModeThreats table for other addons to use
    if TWT.tableSize(TWT.threats) > 0 then
        TWT.synthesizeTankModeData()
    end
...
function TWT.handleTankModePacket(packet)
    ...
    -- Clear only non-synthetic entries when receiving tank mode packets
    for guid, data in pairs(TWT.tankModeThreats) do
        if not data.synthetic then
            TWT.tankModeThreats[guid] = nil
        end
    end
    ...
    for _, tData in players do
        ...
        -- Parse more fields from the tank mode packet
        -- Expected format: creature:guid:name:perc:threat:tps:tank:isTopThreat
        if msgEx[1] and msgEx[2] and msgEx[3] and msgEx[4] then
            local creature = msgEx[1]
            local guid = msgEx[2]
            local name = msgEx[3]
            local perc = __parseint(msgEx[4])
            local threat = msgEx[5] and __parseint(msgEx[5]) or 0
            local tps = msgEx[6] and __parseint(msgEx[6]) or 0
            local tank = msgEx[7] and msgEx[7] == '1' or false
            local isTopThreat = msgEx[8] and msgEx[8] == '1' or false
...
function TWT.combatEnd()
    ...
    TWT.threats = TWT.wipe(TWT.threats)
    -- Clear only synthetic entries on combat end, keep API entries
    for guid, data in pairs(TWT.tankModeThreats) do
        if data.synthetic then
            TWT.tankModeThreats[guid] = nil
        end
...
function TWT.targetChanged()
    ...
    -- Always try to synthesize tank mode data when target changes
    if UnitExists('target') and not UnitIsPlayer('target') then
        -- Don't clear synthetic entries here, let the synthesize function handle it
    end
...
function TWT.updateUI(from)
    ...
    for name, data in TWT.ohShitHereWeSortAgain(TWT.threats, true) do
     ...
     -- Tank mode window handling - only update if elements exist
    if TWT_CONFIG.tankMode and _G['TWTMainTankModeWindow'] then

        -- Hide all TMEF frames if they exist
        for i = 1, 5 do
            local tmef = _G['TMEF' .. i]
            if tmef then
                tmef:Hide()
            end
        end
        ...
        if TWT.tableSize(TWT.tankModeThreats) > 0 then
            ...
            for guid, data in next, TWT.tankModeThreats do
                ...
                local tmef = _G['TMEF' .. i]
                if not tmef then
                    break  -- TMEF frame doesn't exist
                end
                
                _G['TWTMainTankModeWindow']:SetHeight(i * 25 + 23)

                local targetText = _G['TMEF' .. i .. 'Target']
                local playerText = _G['TMEF' .. i .. 'Player']
                local percText = _G['TMEF' .. i .. 'Perc']
                local targetButton = _G['TMEF' .. i .. 'TargetButton']
                
                if targetText then targetText:SetText(data.creature) end
                if playerText then playerText:SetText(TWT.classColors[TWT.getClass(data.name)].c .. data.name) end
                if percText then percText:SetText(TWT.round(data.perc) .. '%') end
                
                -- Only set ID if targetButton exists and has SetID method
                if targetButton and targetButton.SetID then
                    -- Use i as ID since guid might be a string
                    targetButton:SetID(i)
                end
                
                tmef:SetPoint("TOPLEFT", _G["TWTMainTankModeWindow"], "TOPLEFT", 0, -21 + 24 - i * 25)

                local raidIcon = _G['TMEF' .. i .. 'RaidTargetIcon']
                if raidIcon then raidIcon:Hide() end

                local bg = _G['TMEF' .. i .. 'BG']
                if bg then
                    if data.perc >= 0 and data.perc < 50 then
                        bg:SetVertexColor(data.perc / 50, 1, 0, 0.5)
                    else
                        bg:SetVertexColor(1, 1 - (data.perc - 50) / 50, 0, 0.5)
                    end
                end
        ...
        if _G['TWTMainTankModeWindow'] then
            _G['TWTMainTankModeWindow']:Hide()
        end
    end
end
...
TWT.threatQuery:SetScript("OnUpdate", function()
    ...
    if gt >= st then
        ...
        if UnitAffectingCombat('target') then
            ...
            -- Always try to synthesize tank mode data when we get threat updates
            if TWT.tableSize(TWT.threats) > 0 then
                TWT.synthesizeTankModeData()
            end
        end
...
function TWTTargetButton_OnClick(index)
    -- index is a number from SetID, we need to find the corresponding guid
    -- Since we're using i as the ID, we need to find the i-th entry in tankModeThreats
    
    local i = index
    local guid = nil
    local count = 0
    
    -- Find the i-th guid in the table
    for g, _ in pairs(TWT.tankModeThreats) do
        count = count + 1
        if count == i then
            guid = g
            break
    ...
    if guid and TWT.tankModeThreats[guid] then
        local data = TWT.tankModeThreats[guid]
        local unit = TWT.units[guid]
